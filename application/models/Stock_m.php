...
class stock_m extends CI_Model {
    ...
    public function get_stock_in($id = null) {
        $this->db->select('t_stock.stock_id, p_item.barcode, 
                           p_item.nama as item_name, qty, date, detail, 
                           tbl_supplier.nama as supplier_name, p_item.item_id');
        $this->db->from('t_stock');
        $this->db->join('p_item', 't_stock.item_id = p_item.item_id');
        $this->db->join('tbl_supplier', 't_stock.supplier_id = tbl_supplier.supplier_id', 'left');
